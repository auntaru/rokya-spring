...
*/
	...
	public List<Object[]> listCountMonthSum(){
		...
		// String hql = "select count(*) , dataC , sum(valoare) from ComenziVExt where extract(month from dataC) in (7,8,9) and extract(year from dataC)=2015 group by dataC order by dataC";
		String hql = "select count(*) , max(extract( month from dataC)) as luna , sum(valoare) from ComenziVExt where extract(year from dataC)=2015 group by extract( month from dataC) order by luna";
		Query query = getCurrentSession().createQuery(hql);
		return query.list();
	}
	...
	public List<Object[]> listCountMonthSumOnYear(Integer theYear){
		...
		// String hql = "select count(*) , dataC , sum(valoare) from ComenziVExt where extract(month from dataC) in (7,8,9) and extract(year from dataC)=2015 group by dataC order by dataC";
		// String hql = "select count(*) , max(extract( month from dataC)) as luna , sum(valoare) from ComenziVExt where (extract(year from dataC)=extract(year from :dlo)) group by extract( month from dataC) order by luna";
		String hql = "select count(*) , max(extract( month from dataC)) as luna , sum(valoare) from ComenziVExt where extract(year from dataC)=:theYear group by extract( month from dataC) order by luna";
		Query query = getCurrentSession().createQuery(hql);
